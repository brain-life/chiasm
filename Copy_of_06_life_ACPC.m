...
specific_data=strcat('/home/auguser2016/Projects/Chiasm_RJP_FP_ACPC/',subj,'/')
current = pwd

cd(general_data)

% Build the file names for the diffusion data, the anatomical MRI.
dwiFile       = fullfile(strcat(specific_data,subj,'_clean_150mm_aligned_trilin_noMEC.nii.gz'));
dwiFileRepeat = fullfile(strcat(specific_data,subj,'_clean_150mm_aligned_trilin_noMEC.nii.gz'));
t1File        = fullfile(strcat(specific_data,subj,'_t1_acpc.nii.gz'));
fgFileName = fullfile(strcat(specific_data,'Tractography/',subj,'_ACT.tck'));
...
cd(specific_data)
fgWrite(fg,strcat(subj,'_acpc_filtered.mat'))
